<nav class="pagination">
	...
	?>
	<?php	if ($startPageIndex > 0): ?>
		<a href="?<?= http_build_query(array_merge( $_GET, array( 'page' => 1))); ?>">1</a>
	<?php endif;?>
	<?php for ($i = $startPageIndex; $i < $endPageIndex; $i++):?>
		<a class="<?= (($i + 1) == $page) ? "active" : "" ?>" href="?<?= http_build_query(array_merge( $_GET, array( 'page' => $i + 1))); ?>"><?= $i + 1 ?></a>
	<?php endfor; ?>
	<?php	if ($endPageIndex < $totalPage): ?>
		<a href="?<?= http_build_query(array_merge( $_GET, array( 'page' => $totalPage))); ?>"><?= $totalPage ?></a>
	<?php endif;?>
